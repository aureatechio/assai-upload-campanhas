...
CAMPAIGN_SLUG = "dia-imbativel-11-02"
CAMPAIGN_BASE_PATH = Path(
    r"G:\Drives compartilhados\__JOBS 2025\_ASSAI\_ROBO ASSAI\MIDIAS\DIA IMBATIVEL 11_02"
)
...
CAMPAIGNS = [
    {
        "period": "HOJE",
        "region": "NACIONAL",
        "option_sheet": "diaImbativel1102Hoje",
        "display": "Dia Imbativel 11.02 Hoje Nacional",
    },
    {
        "period": "HOJE",
        "region": "MG",
        "option_sheet": "diaImbativel1102HojeMg",
        "display": "Dia Imbativel 11.02 Hoje Mg",
    },
    {
        "period": "AMANHA",
        "region": "NACIONAL",
        "option_sheet": "diaImbativel1102Amanha",
        "display": "Dia Imbativel 11.02 Amanha Nacional",
    },
    {
        "period": "AMANHA",
        "region": "MG",
        "option_sheet": "diaImbativel1102AmanhaMg",
        "display": "Dia Imbativel 11.02 Amanha Mg",
    },
    {
        "period": "GENERICO",
        "region": "NACIONAL",
        "option_sheet": "diaImbativel1102Generico",
        "display": "Dia Imbativel 11.02 Generico Nacional",
    },
    {
        "period": "GENERICO",
        "region": "MG",
        "option_sheet": "diaImbativel1102GenericoMg",
        "display": "Dia Imbativel 11.02 Generico Mg",
    },
]
...
def map_period_dirs(root: Path) -> Dict[str, Path]:
    ...
    for entry in root.iterdir():
        ...
        elif "GENERICO" in name:
            periods["GENERICO"] = entry
    return periods
...
def thumb_by_region(thumb_files: List[Path]) -> Dict[str, Path]:
    mapping: Dict[str, Path] = {}
    for file in thumb_files:
        if "(1)" in file.stem:
            mapping["MG"] = file
        else:
            mapping["NACIONAL"] = file
    if "NACIONAL" not in mapping and thumb_files:
        mapping["NACIONAL"] = thumb_files[0]
    if "MG" not in mapping and len(thumb_files) > 1:
        mapping["MG"] = thumb_files[1]
    return mapping
...
def main() -> int:
    ...
    trilha_by_period: Dict[str, Optional[str]] = {"HOJE": None, "AMANHA": None, "GENERICO": None}
    ...
    for period, period_path in period_dirs.items():
        ...
        bg_root = find_subdir(period_path, "BG") or period_path
        trilha_root = find_subdir(period_path, "TRILHA") or period_path
        thumb_root = find_subdir(period_path, "THUMB") or period_path

        cab_regions = map_region_dirs(cabeca_root)
        ass_regions = map_region_dirs(ass_root)
        bg_regions = map_region_dirs(bg_root)
        thumb_regions = thumb_by_region(scan_files(thumb_root, (".png", ".jpg", ".jpeg")))

        trilha_files = scan_files(trilha_root, (".mp3", ".wav"))
        if trilha_files:
            key = ("trilha", period, "ALL", trilha_files[0].name)
            if key not in uploaded_cache:
                uploaded_cache[key] = upload_and_url(token, CAMPAIGN_SLUG, "trilha", period, "all", trilha_files[0])
            trilha_by_period[period] = uploaded_cache[key]
        ...
        for campaign in CAMPAIGNS:
            ...
            for media in scan_files(cab_regions.get(region, Path()), (".mp4",)):
                ...
                if key not in uploaded_cache:
                    uploaded_cache[key] = upload_and_url(token, CAMPAIGN_SLUG, "cabeca", period, region, media)
                cab_rows.append(
                    {
            ...
            for media in scan_files(ass_regions.get(region, Path()), (".mp4",)):
                ...
                if key not in uploaded_cache:
                    uploaded_cache[key] = upload_and_url(token, CAMPAIGN_SLUG, "assinatura", period, region, media)
                ass_rows.append(
                    {
                        "ligacaoCampanhaFieldName": option,
                        "locucaoTranscrita": "",
            ...
            for fmt in ["16x9", "1x1", "9x16"]:
                ...
                if key not in uploaded_cache:
                    uploaded_cache[key] = upload_and_url(token, CAMPAIGN_SLUG, "background", period, region, media)
                bg_rows.append(
                    {
                        "ligacaoCampanhaFieldName": option,
                        "locucaoTranscrita": "",
            ...
            thumb_file = thumb_regions.get(region)
            if thumb_file:
                key = ("thumb", period, region, thumb_file.name)
                if key not in uploaded_cache:
                    uploaded_cache[key] = upload_and_url(token, CAMPAIGN_SLUG, "thumb", period, region, thumb_file)
                thumb_url_by_option[option] = uploaded_cache[key]

    for period in ["HOJE", "AMANHA", "GENERICO"]:
        options = [c["option_sheet"] for c in CAMPAIGNS if c["period"] == period]
        url = trilha_by_period.get(period)
        if not url:
            continue
        trilha_rows.append(
            {
                "ligacaoCampanhaFieldName": " , ".join(options),
                "locucaoTranscrita": "",
                "nameFile": "FUZZR_ASSAI_IMBATIVEL_TRILHA MAIS BAIXA_33.wav",
    ...
    print(
        f"{CAMPAIGN_SLUG}: cabeca={len(cab_rows)} assinatura={len(ass_rows)} "
